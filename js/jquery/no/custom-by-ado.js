...
var _unitCm='cm';
var _customSizeOptionFlag='egendefinert størrelse'; //自定义size标识
var _jacketOptionFlag='med jacket'; //衣服带夹克标识
...
var _jacketLablesStartWith=["shoulder","armseye","armscye","arm length"];
//元素标识，固定
...
var _optionTypeGuide={1:{"image":"/js/jquery/no/images/guide_weddingdress.png","flag":"waist","bigImage":"/js/jquery/no/images/68-large.png"},2:{"image":"/js/jquery/no/images/guide_jacket.png","flag":"shoulder","bigImage":"/js/jquery/no/images/guide-jacket-large.png"}};
...
function setJecketValueDefault(obj,v){
	jQuery(obj).parents('dd:eq(0)').find('.input-box-'+_jacketLable+'-guide').find('input').each(function() {
        jQuery(this).val(v);
    });	
}
...
function getPriceSpanFromLi(obj,oldPrice){
	...
	/*
	if(oldPrice && jQuery(obj).find('p.old-price').find('span.price').length>0){
		return getPriceFromString(jQuery(obj).find('p.old-price').find('span.price').html());
	}else if(!oldPrice){
		if(jQuery(obj).find('p.special-price').find('span.price').length>0){
			return getPriceFromString(jQuery(obj).find('p.special-price').find('span.price').html());
...
function _cnstra(key){
  var lan =	{"jacket":"Jakke","with jacket":"Med jakke","without jacket":"Uten jakke","shoulder (inch)":"Skulder (tommer)","armseye (inch)":"Armhull (tommer)","armscye (inch)":"Armhull (tommer)","arm length (inch)":"Armlengde (tommer)","measurement unit":"M?ling Unit","custom size":"Egendefinerte m?linger","bust":"Byste","waist":"Midje","hips":"Hofter","hollow to floor":"Hult til Gulvet","inch":"tommers","shawl":"Sjal","with shawl":"Med sjal","without shawl":"Uten sjal","height":"Høyde"};
    key=key.toLowerCase();
	key=jQuery.trim(key);
