<?php
class Ado_Api_WishlistController extends Mage_Core_Controller_Front_Action {
	...
	public function delAction(){
		...
		if (Mage::getSingleton('customer/session')->isLoggedIn()) {
			...
			echo json_encode(
				array(
					'code' => 0,
					'message' => 'delete wish list product '.$product_id.' success!',
					'model' => $this->_getWishlist()
				)
			);
		}else{
	...
	protected function _getWishlist() {
		...
		foreach ( $wishlist->getItemCollection () as $item ) {
			...
			if ($item->getId ()) {
				$price = Mage::getModel('mapi/currency')->getCurrencyPrice(($item->getSpecialPrice()) == null ? ($item->getPrice()) : ($item->getSpecialPrice()));
				$items [] = array (
					'name' => $item->getName (),
