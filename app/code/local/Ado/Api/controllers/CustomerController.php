...
/**
 * Class Ado_Api_CustomerController
 */
class Ado_Api_CustomerController extends Ado_Api_BaseController {
	...
	public function registerAction() {
		...
		try {
			...
			if ($e->getCode () === Mage_Customer_Model_Customer::EXCEPTION_EMAIL_EXISTS) {
				...
				$message = $e->getMessage ().'=1='.print_r($params,true);
			}
			echo json_encode ( array (
					'code'=>1,
					'message'=>$message.'=2='.print_r($params,true),
					'model'=>array ()
			) );
		} catch ( Exception $e ) {
			echo json_encode ( array (
					'code'=>1,
					'message'=>$e->getMessage ().'=3='.print_r($params,true),
					'model'=>array ()
	...
	public function logoutAction() {
		try {
			...
			echo json_encode(array('code'=>0, 'message'=>null, 'model'=>array()));
		} catch (Exception $e) {
			echo json_encode(array('code'=>1, 'message'=>$e->getMessage(), 'model'=>array()));
		}
	}
	 ...
	 * @return bool
	 */
	public function isexistsAction($email) { 
		$email = Mage::app ()->getRequest ()->getParam ( 'email' );
		$info ='Email is enable.';
		$uname_is_exist = $this->_user_isexists($email);  
		if($uname_is_exist)$info='Email is exist. Please change email and retry!';
		$result = array (
				'code' => $uname_is_exist,
				'is_exists'=>$uname_is_exist,
				'message' => $info,
				'model'=>array()
		);
	...
	public function updateAccountAction(){
		if (Mage::getSingleton ( 'customer/session' )->isLoggedIn()) {
	        ...
	        if (!empty($errors)) {
	            echo json_encode(array('code'=>1, 'message'=>$errors, 'model'=>array()));
	            return ;
	        }
	        try {
	        	$customer->save();
	        	echo json_encode(array('code'=>0, 'message'=>'success', 'model'=>$userData));
	        } catch (Mage_Core_Exception $e){
	        	echo json_encode(array('code'=>1, 'message'=>$e->getMessage(), 'model'=>array()));
	        } catch (Exception $e){
	        	echo json_encode(array('code'=>2, 'message'=>$e->getMessage(), 'model'=>array()));
	        }
	    } else {
	    	echo json_encode(array(
				'code' => 5,
	...
	public function getAccountInfoAction(){
		if (Mage::getSingleton('customer/session')->isLoggedIn()) {
			...
			echo json_encode(array('code'=>0, 'message'=>'get customer info success!', 'model'=>$userData));
		}else{
			echo json_encode(array(
				'code' => 5,
				'message' => 'not user login',
	...
	public function updatePasswordAction()
	{
		if (Mage::getSingleton('customer/session')->isLoggedIn()) {
			...
			if($password && $new_password && ($validate == 0)){
				...
				try {
					$customer->save();
					echo json_encode(array('code' => 0, 'message' => 'success', 'model' => array()));
				} catch (Mage_Core_Exception $e) {
					echo json_encode(array('code' => 1, 'message' => $e->getMessage(), 'model' => array()));
				} catch (Exception $e) {
					echo json_encode(array('code' => 2, 'message' => $e->getMessage(), 'model' => array()));
				...
				echo json_encode(array('code' => 2, 'message' => 'password is not correct ', 'model' => array()));
			}
		}else {
			echo json_encode(array(
				'code' => 5,
