...
class Ado_Api_CheckoutController extends Mage_Core_Controller_Front_Action{
    ...
    public function getAddressListAction(){
        ...
        $addressList = Mage::getModel('mapi/address')->getCustomerAddressList($customer);
    ...
    public function getAddressByQuoteAction(){
        $quote = Mage::getSingleton('checkout/session')->getQuote();
        if($quote->getId()){
            $address = Mage::getModel('mapi/checkout')->getAddressByQuote($quote);
            echo json_encode(array(
                'code'=>0,
    ...
    public function getShippingMethodsListAction(){
        ...
        $return_result['model'] = Mage::getModel('mapi/checkout')->getShippingMethodListByQuote($quote);
        echo json_encode($return_result);
    ...
    public function getPayMethodsListAction(){
        $quote = Mage::getSingleton('checkout/session')->getQuote();
        $payment_methods = Mage::getModel('mapi/checkout')->getActivePaymentMethods($quote);
        echo json_encode(array(
            'code'=> 0,
    ...
    public function getOrderReviewAction(){
        ...
        foreach ($quote->getAllVisibleItems() as $item) {
            ...
            if($item->getProductType()=='configurable'){
                $cartItemArr['custom_option'] =  Mage::getModel('mapi/cart')->getCustomOptions($item);
            }
            ...
            $exclPrice = Mage::helper('mapi')->formatPriceForXml($exclPrice);
            $formatedExclPrice = $quote->getStore()->formatPrice($exclPrice, false);
            $inclPrice = Mage::helper('mapi')->formatPriceForXml($inclPrice);
...
//        $orderReviewArr['order_id'] = $this->getOnepage()->getLastOrderId();
        $orderReviewArr['coupon'] = Mage::getModel('mapi/checkout')->getCouponByQuote($quote);
        $orderReviewArr['pay_method'] = Mage::getModel('mapi/checkout')->getPaymentMethodByQuote($quote);
        $orderReviewArr['address'] = Mage::getModel('mapi/checkout')->getAddressByQuote($quote);
//        $orderReviewArr['selected_shipping_method_id'] = $quote->getShippingAddress()->getShippingMethod();
        ...
        $orderReviewArr['base_discount_amount'] = number_format(Mage::getModel('mapi/currency')->getCurrencyPrice($quote->getShippingAddress()->getBaseDiscountAmount()),2,'.','');
        $orderReviewArr['grand_total'] = $quote->getShippingAddress()->getGrandTotal();
        $orderReviewArr['shipping_method'] = Mage::getModel('mapi/checkout')->getShippingMethodByQuote($quote);
    ...
    public function successAction()
    {
        ...
        $order_info = Mage::getModel('mapi/order')->getOrderByEntityId($lastOrderId);
        //$session->clear();
