 ...
 */
class Ado_Api_AddressController extends Mage_Core_Controller_Front_Action
{
    ...
    public function getAddressAction(){
        ...
        $return_address = Mage::getModel('mapi/address')->getAddressById($addressId);
        echo json_encode(
            array(
                'code' => 0,
        ...
        if (!is_null($addressData)) {
            ...
            try {
                ...
                if (true === $addressValidation) {
                    $address->save();
                    $result['code'] = 0;
                    $result['message'] = 'save or update user address success!';
                    echo json_encode($result);
                    return;
                } else {
                    if (is_array($errors)) {
                        ...
                        $result['code'] = 3;
                        $result['message'] = 'Can\'t save or update address';
                    }
                    echo json_encode($result);
                    return;
                }
            ...
            $result['code'] = 6;
            $result['message'] = 'address data is null!';
            echo json_encode($result);
            return;
        }
    }
