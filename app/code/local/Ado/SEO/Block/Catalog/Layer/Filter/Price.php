 ...
 */
class Ado_SEO_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price
{
    ...
    public function __construct()
    {
        ...
        if ($this->helper('ado_seo')->isEnabled()
            && $this->helper('ado_seo')->isPriceSliderEnabled()) {
            // Modify template to render price filter as slider
            $this->setTemplate('ado_seo/catalog/layer/price.phtml');
        }
    }
    ...
    {
        $type = $this->helper('ado_seo')->getPriceSliderSubmitType();

        if ($type == Ado_SEO_Model_System_Config_Source_Slider_Submit_Type::SUBMIT_BUTTON) {
            return true;
        }
