...
class Dock_Rural_Model_Import_Cms extends Mage_Core_Model_Abstract
{
    const ITEM_TITLE_PREFIX = 'Rural ';
    ...
    {
        parent::__construct();
        $this->_basePath = Mage::getBaseDir() . '/app/code/local/Dock/Rural/etc/importexport/cms/';
    }
    ...
    public function importCmsItems($modelString, $entityName, $demoNumber, $overwrite = false)
    {
        ...
        try {
            if (!is_readable($xmlFilePath)) {
                throw new Exception(
                    Mage::helper('rural')->__("Can't read data file: %s", $xmlFilePath)
                );
            }
            ...
            if ($i) {
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('rural')->__('Number of imported items: <strong>%s</strong>. Items with the following identifiers were imported:<br />%s', $i, implode(', ', array_keys($alreadyImportedItems)))
                );
            } else {
                Mage::getSingleton('adminhtml/session')->addNotice(
                    Mage::helper('rural')->__('No items were imported.')
                );
            }
