...
class Dock_Rural_Model_Observer
{
    /**
     ...
     * @return Dock_Rural_Model_Observer
     */
    ...
    public function hookTo_controllerActionPostdispatchAdminhtmlSystemConfigSave()
    {
        ...
        if ($section == 'rural_layout') {
            $websiteCode = Mage::app()->getRequest()->getParam('website');
            $storeCode = Mage::app()->getRequest()->getParam('store');

            $cg = Mage::getSingleton('rural/cssgen_generator');
            $cg->generateCss('grid', $websiteCode, $storeCode);
            $cg->generateCss('layout', $websiteCode, $storeCode);
        } elseif ($section == 'rural_design') {
            $websiteCode = Mage::app()->getRequest()->getParam('website');
            $storeCode = Mage::app()->getRequest()->getParam('store');

            Mage::getSingleton('rural/cssgen_generator')->generateCss('design', $websiteCode, $storeCode);
        } elseif ($section == 'rural') {
            $websiteCode = Mage::app()->getRequest()->getParam('website');
            $storeCode = Mage::app()->getRequest()->getParam('store');

            Mage::getSingleton('rural/cssgen_generator')->generateCss('layout', $websiteCode, $storeCode);
        }
    }
