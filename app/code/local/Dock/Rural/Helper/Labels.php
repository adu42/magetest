...
class Dock_Rural_Helper_Labels extends Mage_Core_Helper_Abstract
{
	/**
	...
	public function getLabels($product)
	{
		...
		$isNew = false;
		if (Mage::getStoreConfig('rural/product_labels/new'))
		{	
			$isNew = $this->isNew($product);
		}
		
		$isSale = false;
		if (Mage::getStoreConfig('rural/product_labels/sale'))
		{
		...
		if ($isNew == true)
		{
			$html .= '<span class="sticker-wrapper bottom-right"><span class="sticker new">' . $this->__('New') . '</span></span>';
		}
		
		if ($isSale == true)
		{
			$html .= '<span class="sticker-wrapper bottom-right"><span class="sticker sale">' . $this->__('Sale') . '</span></span>';
		}
