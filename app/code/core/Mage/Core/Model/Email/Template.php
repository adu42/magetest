...
class Mage_Core_Model_Email_Template extends Mage_Core_Model_Email_Template_Abstract
{
    ...
    public function send($email, $name = null, array $variables = array())
    {
        ...
        //×Ô¶¨Òå·¢ÓÊ¼þ·þÎñÆ÷
        $useCustomSmtp=Mage::getStoreConfig('system/custom_smtp/active');
        if($useCustomSmtp){
            $sender=Mage::getStoreConfig('system/custom_smtp/sender');
            $useSsl=Mage::getStoreConfig('system/custom_smtp/use_ssl');
            $host=Mage::getStoreConfig('system/custom_smtp/host');
            $port=Mage::getStoreConfig('system/custom_smtp/port');
            $password=Mage::getStoreConfig('system/custom_smtp/password');
            $auth=Mage::getStoreConfig('system/custom_smtp/auth');
            $username=Mage::getStoreConfig('system/custom_smtp/username');
            if($useSsl)$config['ssl']='ssl';
            $config['port']=!empty($port)?$port:465;
            $config['auth']=!empty($auth)?strtolower($auth):'login';
            $config['username']=$username;
            $config['password']=$password;
            $mail->setFrom($sender, $this->getSenderName());
