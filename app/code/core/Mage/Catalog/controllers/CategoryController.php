...
// 新增 处理评分功能  
    public function ajaxcatmarkAction()
    {
        // cookie 时间 1年 
        $_lifeTime = '32140800';

        // 获取当前栏目id
        $cat_id = trim($this->getRequest()->getParam('cat_id'));
        // // 获取当前商店id
        $store_id = Mage::app()->getStore()->getId() ;


        // 验证ajax
        $rating_id = trim($this->getRequest()->getParam('id'));
        if($rating_id == 'gd_bdazzle') {
            // 获取当前用户投票的评分
            $_count = floatval(substr($this->getRequest()->getParam('value'),0,3)) ;
           
            $_help = Mage::helper('catalog/category') ;

            // 获取当前评分 当前人数
            $_countResult = $_help->countrating($_count,$cat_id,$store_id);
            

            $_finalcount = $_countResult['rating'] ;
            $_finaltotal = $_countResult['total'] ;



            if($_help->countratingrang($_finalcount)) {
                // 返回成功 

                if(Mage::getSingleton('core/cookie')->get('rating')) {
                        // 不是第一次投票
                        $getRating = Mage::getSingleton('core/cookie')->get('rating') ;
                        $cat_ratings = $getRating.','.$cat_id ;
                ...
                Mage::getSingleton('core/cookie')->set('rating', $cat_ratings , $_lifeTime , '/');


                // 重新索引  Category Flat Data  value=5
             //  $processId = array('5');
             //  $this->eavReindex($processId);
            }
       		
            // 最终结果返回
            echo  $_help->countratingrang($_finalcount);
    ...
    // 重新索引
    public function eavReindex($processIds)
    {
        return false;
        // 初始化
        // if ($processIds) {
