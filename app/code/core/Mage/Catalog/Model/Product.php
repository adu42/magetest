...
/**
     * by@ado
     * Retrieve current prev product 
     **@return array or  0
     */
	public function _getPrevProduct($id){
		$read=Mage::getSingleton('core/resource')->getConnection('core_read');
		$sql="select entity_id from  catalog_product_entity where entity_id < '$id' limit 1";
        return $read->fetchOnce($sql);
	}

   public function _getNextProduct($id){
		$read=Mage::getSingleton('core/resource')->getConnection('core_read');
		$sql="select entity_id from  catalog_product_entity where entity_id > '$id' limit 1";
        return $read->fetchOnce($sql);
	}

	protected function _getProductMes($id){
			$str='';
			$storeId=Mage::app()->getStore()->getStoreId();
			$product = $this->setStore($storeId)->load($id);
			if($product && $product->getId()){
				$name = $product->getName(); 
				$url =  Mage::getBaseUrl().$product->getUrlPath();
				$str=sprintf('<a href="%s">%s</a>',$url,$name);
			}
		return $str;
	}
	...
	public function getSoldProducts(){
		...
		$sql="select order_id,sku from  sales_flat_order_item group by sku order by item_id DESC limit 10";
		$arr=$read->fetchAll($sql);
		$arru = array();
		foreach($arr as $key=>$v){
			$sku_arr = explode('-',$v['sku']);
			$sku = $sku_arr[0];
			$sql1 = "select entity_id from catalog_product_entity where sku ='".$sku."' limit 1";
			$res = $read->fetchAll($sql1);
            $sql = "select attribute_id from eav_attribute where entity_type_id=4 and attribute_code='name' limit 1";
            $name_attribute_id=$read->fetchOne($sql);
            $sql = "select attribute_id from eav_attribute where entity_type_id=4 and attribute_code='url_path' limit 1";
            $url_attribute_id=$read->fetchOne($sql);
			if(!empty($res)){
				
				$product = $this->setStore($storeId)->load($res[0]['entity_id']);

				if($product && $product->getId()){
					$arru[$key]['name'] = $product->getName();
					$arru[$key]['url'] = Mage::getBaseUrl().$product->getUrlPath();
					
					$sql3 = "select * from sales_flat_order_address where parent_id ='".$v['order_id']."' limit 1";
					$arrc = $read->fetchAll($sql3);

				$country = $this->getCountry($arrc[0]['country_id']);
				if($country){
					if($arrc[0]['region']){
						$region = $arrc[0]['region'].",".$country;
					}else{
						$region = $country;
					}
				}
				$arru[$key]['address'] = $region;
