...
class IWD_Productvideo_Model_Productvideo extends Mage_Core_Model_Abstract
{
    ...
    // 获取video信息 Mobile 
    public function getVideoInfo($image)
    {

        $table_name = 'iwd_video';           
        // $store_id = Mage::app()->getStore()->getId() ;
        // $store_id = unserialize($store_id);
        $conn = Mage::getSingleton('core/resource')->getConnection('core_read');  
        //SQL
        $results = $conn->fetchAll("SELECT * FROM  ".$table_name. " WHERE   image ='" . $image . "' limit 1 ");  
