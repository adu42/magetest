        ...
        */
        ...
        if($mailchimpListId && $mailchimp){
            ...
            if($api->errorCode){              
              if($api->errorCode==214){
                $ajaxExceptions['exceptions'][] = utf8_encode($this->__('You are already subscribed to our newsletter'));
              }else{
        ...
        if($anSegmentCode){
            $subscriber = Mage::getModel('advancednewsletter/subscriber')->loadByEmail($email);
              ...
              $params['skip_guest_check'] = true;
              $subscriber = Mage::getModel('advancednewsletter/subscriber')->subscribe($email, $segmentsToSubscribe, $params);
            }                                                    
        }                                                      
